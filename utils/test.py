...
from utils.imager import plot, TRANSPOSE, ROTATE, OFFSET
...
def data_test(test_data_map, set_type, embeddings, sess, test_ops, log=False):
    cnt = 0
    total = 0
    results = []
...
def test():
    ...
    with graph.as_default():
        ...
        with tf.Session(graph=graph, config=config) as sess:
            ...
            res, rate = data_test(test_data_map, "test", embeddings, sess, test_ops, log=True)
            print(rate)
