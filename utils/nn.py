...
def random_mini_batches(data_set, simple_indices, mini_batch_size = 64, seed = 0):
    """ 切分训练集为 mini_batch """
    np.random.seed(seed)
    ...
    for batch_permutation in batch_permutation_indices:
        ...
        simple_indices_permutation = list(np.random.permutation(len(simple_indices)))
        mini_batch_simple_indices = [
            simple_indices[simple_indices_permutation[: mini_batch_size]],
            simple_indices[simple_indices_permutation[mini_batch_size: mini_batch_size*2]]
            ]
        yield mini_batch_X, mini_batch_simple_indices
...
def nucleus_loss(nucleus1, nucleus2, radius = 0.1):
    """ 计算二元组损失 """
    dist = tf.reduce_sum(tf.square(tf.subtract(nucleus1, nucleus2)), axis=-1)
    basic_loss = tf.add(2 * tf.maximum(tf.subtract(radius, dist), 0), tf.maximum(tf.subtract(dist, radius * 2), 0))
