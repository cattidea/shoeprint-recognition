...
def model_132_48_inception(X, mask, is_training, keep_prob):
    ...
    A4 = inception_v2(input=A4, scope="INCEPTION3", filters=128, is_training=is_training)
    A4 = inception_v2(input=A4, scope="INCEPTION4", filters=128, is_training=is_training)
    print("A4: {}".format(A4.shape))
...
def model_132_48_inception_v2(X, mask, is_training, keep_prob):
    A0 = X
    print("A0: {}".format(A0.shape))
    ...
    # INCEPTION L1
    A3 = inception_v2(input=A2, scope="INCEPTION1", filters=32, is_training=is_training)
    print("A3: {}".format(A3.shape))

    # INCEPTION L2
    A4 = inception_v2(input=A3, scope="INCEPTION2", filters=64, is_training=is_training)
    print("A4: {}".format(A4.shape))

    # INCEPTION L3
    A5 = inception_v2(input=A4, scope="INCEPTION3", filters=128, is_training=is_training)
    print("A5: {}".format(A5.shape))

    # INCEPTION L4
    A6 = inception_v2(input=A5, scope="INCEPTION4", filters=256, is_training=is_training)
    print("A6: {}".format(A6.shape))

    # INCEPTION L5
    A7 = inception_v2(input=A6, scope="INCEPTION5", filters=512, is_training=is_training)
    print("A7: {}".format(A7.shape))

    # INCEPTION L6
    A8 = inception_v2(input=A7, scope="INCEPTION6", filters=512, is_training=is_training)
    print("A8: {}".format(A8.shape))

    # CONV to 1
    A9 = A8 * mask
    A9 = tf.layers.conv2d(inputs=A9, filters=512, kernel_size=(7, 5), strides=3, padding='valid',
                           kernel_initializer=tf.truncated_normal_initializer(stddev=0.1), name="CONV_to_6_2", reuse=tf.AUTO_REUSE)
    A9 = tf.layers.conv2d(inputs=A9, filters=512, kernel_size=(6, 2), strides=1, padding='valid',
                           kernel_initializer=tf.truncated_normal_initializer(stddev=0.1), name="CONV_to_1_1", reuse=tf.AUTO_REUSE)
    # A9 = tf.nn.relu(A9)
    A9 = maxout(A9, num_units=128)
    print("A9: {}".format(A9.shape))

    # flatten
    A10 = tf.layers.flatten(A9)
