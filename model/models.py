...
class ModelV3(TripletModel):
    ...
    def model(self, X, is_training, keep_prob):
        ...
        # CONV L1
        X = self.conv2d(X, scope="CONV_1", filter=64, kernel_size=3, strides=1,
                        padding="same", batch_norm=True, activation=tf.nn.relu, is_training=is_training)
        X = tf.layers.max_pooling2d(X, pool_size=3, strides=2, padding="same")
        ...
        # XCEPTION L12
        Res = self.conv2d(X, scope="CONV_res_12", filter=512,
                          kernel_size=1, strides=2, padding="same")
        X = tf.nn.relu(X)
        X = self.separable_conv2d(X, scope="XCEPTION_12a", filter=256, kernel_size=3, strides=1,
                                  padding="same", batch_norm=True, activation=tf.nn.relu, is_training=is_training)
        X = self.separable_conv2d(X, scope="XCEPTION_12b", filter=512, kernel_size=3, strides=1,
                                  padding="same", batch_norm=True, activation=None, is_training=is_training)
        X = tf.layers.max_pooling2d(X, pool_size=3, strides=2, padding="same")
        X = X + Res

        # XCEPTION L13
        X = self.separable_conv2d(X, scope="XCEPTION_13", filter=64, kernel_size=3, strides=1,
                                  padding="same", batch_norm=True, activation=tf.nn.relu, is_training=is_training)

        # dw conv to 1x1
        X = self.deepwise_conv2d(X, scope="dw_conv_to_1_1", channel_multiplier=4, kernel_size=(
            X.shape[1], X.shape[2]), strides=1, padding="valid")
...
class ModelV5(TripletModel):
    ...
    def model(self, X, is_training, keep_prob):
        ...
        X = self.nnet_v1(input=X, filters=16, scope="Nnet_2a", strides=1,
                         activate=True, batch_norm=True, is_training=is_training)
        X = self.nnet_v1(input=X, filters=16, scope="Nnet_2b", strides=1,
                         activate=True, batch_norm=True, is_training=is_training)
        X = self.nnet_v1(input=X, filters=32, scope="Nnet_2c", strides=2,
                         activate=True, batch_norm=True, is_training=is_training)
        # mask = tf.layers.max_pooling2d(mask, pool_size=3, strides=2, padding="same")
        # X = tf.cast(tf.greater(mask, 0), tf.float32) * X
        ...
        X = self.nnet_v1(input=X, filters=32, scope="Nnet_3a", strides=1,
                         activate=True, batch_norm=True, is_training=is_training)
        X = self.nnet_v1(input=X, filters=32, scope="Nnet_3b", strides=1,
                         activate=True, batch_norm=True, is_training=is_training)
        X = self.nnet_v1(input=X, filters=32, scope="Nnet_3c", strides=1,
                         activate=True, batch_norm=True, is_training=is_training)
        X = self.nnet_v1(input=X, filters=64, scope="Nnet_3d", strides=1,
                         activate=True, batch_norm=True, is_training=is_training)
        X = self.nnet_v1(input=X, filters=64, scope="Nnet_3e", strides=1,
                         activate=True, batch_norm=True, is_training=is_training)
        X = self.nnet_v1(input=X, filters=64, scope="Nnet_3f", strides=1,
                         activate=True, batch_norm=True, is_training=is_training)

        # Nnet L4
        X = self.nnet_v1(input=X, filters=64, scope="Nnet_4a", strides=1,
                         activate=True, batch_norm=True, is_training=is_training)
        X = self.nnet_v1(input=X, filters=64, scope="Nnet_4b", strides=1,
                         activate=True, batch_norm=True, is_training=is_training)
        X = self.nnet_v1(input=X, filters=64, scope="Nnet_4c", strides=1,
                         activate=True, batch_norm=True, is_training=is_training)
        X = self.nnet_v1(input=X, filters=128, scope="Nnet_4d", strides=1,
                         activate=True, batch_norm=True, is_training=is_training)
        X = self.nnet_v1(input=X, filters=128, scope="Nnet_4e", strides=1,
                         activate=True, batch_norm=True, is_training=is_training)
        X = self.nnet_v1(input=X, filters=128, scope="Nnet_4f", strides=2,
                         activate=True, batch_norm=True, is_training=is_training)
        # mask = tf.layers.max_pooling2d(mask, pool_size=3, strides=2, padding="same")
        # X = tf.cast(tf.greater(mask, 0), tf.float32) * X
        ...
        X = self.nnet_v1(input=X, filters=128, scope="Nnet_5e", strides=1,
                         activate=True, batch_norm=True, is_training=is_training)
        X = self.nnet_v1(input=X, filters=128, scope="Nnet_5f", strides=2,
                         activate=True, batch_norm=True, is_training=is_training)
        # mask = tf.layers.max_pooling2d(mask, pool_size=3, strides=2, padding="same")
        # X = tf.cast(tf.greater(mask, 0), tf.float32) * X
