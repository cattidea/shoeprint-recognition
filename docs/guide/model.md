...
本程序在神经网络的结构与深度上进行了若干次尝试，从基本的 `vgg-16` 开始，进而使用 `inception_v2` 模块，之后通过研读 CVPR 论文获悉到前几年刚刚提出的新模型，比如 `Xception` 、 `ResNeXt` 、`MobileNet`等等，经过测试，最终保留了 `Inception_v2` 、 `Xception` 与本人自行尝试设计的一个网络（暂时命名为 `Nnet_v1`），其中以 `Nnet_v1` 效果最佳，但由于经验不足与测试比较少，并不代表该网络真的超越了其他网络（在想 Peach）
...
| INCEPTION_4a | (17, 7, 256)  | (3, 3, 256), s=1, "same" | (17, 7, 256)  |
...
-  Average_pool 等价于卷积核全为 1 的可分离卷积，所以可分离卷积会比 Average_pool 做的更好（参考 MobileNet）
-  $f \times f$ 的普通卷积等价于 $f \times f$ + $1 \times 1$ 的普通卷积，且效果更好（参考 Xce）
...
-  BN 使得层与层之间解耦
-  Maxpool 与 Maxout 都有着比较好的效果
-  ResNets 能够保留前层特征用于后层的组合，以学习到更丰富的特征
-  Inception Xception 以及 ResNeXt 都对通道进行了一定的分离，取得了可观的效果

基于以上事实，对输入的 Feature Map 首先进行 $1 \times 1$ 卷积降低通道数，形成瓶颈层以降低运算量，之后对瓶颈层进行 $3 \times 3$ 可分离卷积，同时从输入计算残差，将残差与可分离卷积结果在通道轴上拼接

如果该层需要对 size 进行缩减的话，对刚刚拼接的结果进行两次 $1 \times 1$ 卷积，两个结果分别进行最大池化与可分离卷积，然后在通道上进行拼接

如果该层需要非线性激活的话，对刚刚拼接的结果进行两次 $1 \times 1$ 卷积，对其中一个结果进行 maxout 激活（k = 4），然后与另一个结果在通道轴上进行拼接

最后对结果进行 Batch_Normalization
...
虽然想了不少，但是很多都是自己的一些猜想，暂时无从也无时间进行专业考证，有机会会继续优化并使用 ImageNet 测试实际效果

## 一些弃用的想法

-  使用 mask 机制，也即求出图片的残缺模板，在网络的接近输出层进行过滤，将残缺部位的特征过滤掉，以提高残缺图像的识别率，但是由于效果并不好，弃用
-  更深的网络深度，但是更深的网络使得每个 batch 会更小，模型更难收敛，经 ModelV3、ModelV5 的测试，取消该方案
-  最后的可分离卷积使用多核可分离卷积，也即原来每个通道卷积为一个标量，现在卷积为一个向量，之后再进行后续处理，ModelV3 保留了一个简单实现，但由于效果不好，后续没时间在其他模型测试
